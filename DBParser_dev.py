...
#         """ % (runNumber)
    ...
    def getLatestRunInfo(self):
        query="""SELECT MAX(A.RUNNUMBER), MAX(B.LUMISECTION) 
        FROM CMS_RUNINFO.RUNNUMBERTBL A, CMS_RUNTIME_LOGGER.LUMI_SECTIONS B WHERE B.RUNNUMBER=A.RUNNUMBER AND B.LUMISECTION > 0
        """
        try:
            self.curs.execute(query)
            runNumber, lumiSection = self.curs.fetchone()
        except:
            print "db query error: Unable to retrieve latest run number."
            return
